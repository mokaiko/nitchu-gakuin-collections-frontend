import { BrowserProvider, Contract, JsonRpcProvider, type Eip1193Provider, type Provider, type Signer } from 'ethers';
...
export type CollectionView = {
  ...
  isActive: boolean;
  svgChunkCount: number;
  isSvgFinalized: boolean;
  svg?: string;
  imageSrc?: string;
  hasClaimed?: boolean;
...
export type ResolvedNetwork = ReturnType<typeof getNetworkConfig>;
...
export function getBrowserProvider(): BrowserProvider | undefined {
...
export function getContract(chainId?: number, providerOrSigner?: Provider | Signer): Contract {
  const network = getNetworkConfig(chainId);
...
export function buildImageSrc(svg: string): string {
  return `data:image/svg+xml;utf8,${encodeURIComponent(svg)}`;
}

export function extractImageFromUri(uri: string): string | undefined {
...
export async function listCollectionIds(contract: Contract): Promise<number[]> {
  ...
  if (upperBound === 0) return sequential;
...
export async function loadCollections(contract: Contract, ids: number[], account?: string): Promise<CollectionView[]> {
